...
from src.repository.users import user_cr_info, load_plans, check_mn_plans, check_yr_plans
...
async def user_credits_info(user_id: int,
...
async def download_plans(file: UploadFile = File(None),
                        db: Session = Depends(get_db)):
    if not file.filename.lower().endswith((".xlsx",".xls")):
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Upload xlsx or xls file")
    return await load_plans(file, db)


@app.get("/plans_performance", status_code=status.HTTP_200_OK)
async def check_month_plans(date_on: str = Query(None, description="Date in format 01.12.2022"),
                            db: Session = Depends(get_db)):
...
async def check_year_plans(year_on: str = Query(None, description="Year in format 2022"),
                            db: Session = Depends(get_db)):
